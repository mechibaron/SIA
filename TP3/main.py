...
def main2_lineal(learning_rate, epochs, bias, beta):
  min_out=0
  max_out=100
  X = []
  Z = []
...
# def main2_no_lineal(operation, learning_rate, epochs, bias):
#   X = []
#   Z = []
#   with open('./data/TP3-ej2-conjunto.csv', 'r') as csvfile:
#   reader = csv.DictReader(csvfile)
...
if __name__ == '__main__':
  ...
  # learning rate = eta (n)
  operation, learning_rate, epochs, bias, beta, type_perceptron = utils.getDataFromFile(data)
  if(type_perceptron == 'escalon'):
    main1(operation, learning_rate, epochs, bias)
  elif (type_perceptron == 'lineal'):
    main2_lineal(learning_rate, epochs, bias, beta)
